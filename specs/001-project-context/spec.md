...
2. **Given** `ROUND(2.5, 0)`,
   **When** evaluated,
   **Then** the result is 3 (half-up rounding).
...
- **FR-005**: The formula engine MUST support `IF(condition, true_value, false_value)` where
  condition supports relational operators `=`, `<>`, `<`, `<=`, `>=`, and context-sensitive `>`:
  when `>` appears with a non-cell right-hand operand it is a greater-than comparison; when both
  operands are cell addresses it retains range-sum semantics (existing behavior). true_value and
...
- ROUND uses `Math.Round` with `MidpointRounding.AwayFromZero`.
