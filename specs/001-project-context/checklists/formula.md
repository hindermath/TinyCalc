...
- [ ] CHK001 — Does the spec define behavior when ROUND's `decimals` argument is a
  non-integer value (e.g., `ROUND(3.14, 1.5)`)? [Gap, Spec §Edge Cases]

- [ ] CHK002 — Does the spec define behavior when MIN/MAX/AVERAGE/COUNT receive a numeric
  literal instead of a range or cell reference (e.g., `MIN(42)`)? [Gap]

- [ ] CHK003 — Does the spec define whether IF's condition requires a relational operator, or
  whether any non-zero numeric expression is treated as true (e.g., `IF(A1, 1, 0)`)? [Gap,
  Spec §FR-005]

- [ ] CHK004 — Does the spec define behavior for a range expression inside an IF branch, e.g.,
  `IF(A1>0, A1>B5, 0)` where the true-branch result is a range sum? [Gap, Spec §US3]

- [ ] CHK005 — Does the spec define what happens when COUNT is applied to a range containing
  formula cells whose evaluation produced an error? [Gap, Spec §FR-004]

- [ ] CHK006 — Does SC-005 specify the target location within `docs/help/microcalc-help.md`
  (section, page, or page-break position) where the 6 new functions must be documented?
  [Completeness, Spec §SC-005]

- [ ] CHK007 — Does the spec define whether parentheses are permitted in the IF condition
  expression, e.g., `IF((A1+B1)>100, 1, 0)`? [Gap, Spec §FR-005]

- [ ] CHK008 — Does the spec define behavior when the `decimals` argument of ROUND is itself
...
- [ ] CHK009 — Is the context-sensitive `>` disambiguation rule in FR-005 precise enough for
...
- [ ] CHK010 — Is "cell address" precisely defined in FR-005 for the disambiguation rule — does
  it mean only a bare cell reference (`A1`) or any sub-expression that resolves to a cell value?
  [Clarity, Spec §FR-005]

- [ ] CHK011 — Is "successfully calculated formula value" in FR-004 (COUNT) precisely defined:
  does it include formula cells whose result is exactly 0, or only non-zero results? [Clarity,
  Spec §FR-004]

- [ ] CHK012 — Is "half-up rounding" in FR-006 unambiguous for negative numbers? Is
...
- [ ] CHK013 — Is "numeric cells" in FR-001/FR-002/FR-003/FR-004 precisely defined: does it
...
- [ ] CHK014 — Are the specific German error message strings in the Edge Cases section the
...
- [ ] CHK015 — Does FR-011 ("error messages in German with a position indicator") define the
...
- [ ] CHK016 — Is the edge-case decision that MIN/MAX on an all-empty range returns 0
...
- [ ] CHK017 — Are the error message requirements in FR-011 (German + position indicator)
...
- [ ] CHK018 — Does the context-sensitive `>` rule in FR-005 (IF condition) remain consistent
...
- [ ] CHK019 — Are FR numbering and ordering consistent — FR-007 and FR-008 (IF-specific rules)
...
- [ ] CHK020 — Is SC-001 ("receive correct results for all numeric cells") measurable without
...
- [ ] CHK021 — Is SC-005 ("documented with a usage example") measurable: is a minimum
  acceptable example defined (e.g., must include: function syntax, sample input, expected
  output)? [Measurability, Spec §SC-005]

- [ ] CHK022 — Is SC-007 ("at least one golden test per function covering the happy path and at
...
- [ ] CHK023 — Do the acceptance scenarios in US1 include a rectangular range case where column
...
- [ ] CHK024 — Are requirements defined for function composition, e.g., `ROUND(MIN(A1>A5), 2)`,
  `IF(AVERAGE(A1>A5)>10, 1, 0)`, `COUNT(A1>A5)+MIN(A1>A5)`? [Coverage, Spec §US3, §US4]

- [ ] CHK025 — Is there a scenario defined where the context-sensitive `>` rule is exercised
...
- [ ] CHK026 — Are requirements defined for the cyclic-reference scenario specific to the new
  functions — e.g., `MIN(A1>A5)` where A3 contains a formula referencing the MIN cell? [Coverage,
  Spec §FR-010]

- [ ] CHK027 — Is there a scenario defined for AVERAGE/MIN/MAX/COUNT applied to the full grid
...
- [ ] CHK028 — Does the spec define behavior for `ROUND(value, 0)` — rounding to zero decimal
...
- [ ] CHK029 — Does the spec define behavior for IF with identical true and false branches
  (e.g., `IF(A1>0, 5, 5)`) — is this valid, and does it trigger any degenerate parsing path?
  [Edge Case]

- [ ] CHK030 — Does the spec define behavior when an aggregate function's range includes the
...
- [ ] CHK031 — Does the spec address behavior when `COUNT` is called on a range where all cells
...
- [ ] CHK032 — Are backward-compatibility requirements stated: do existing `.mcalc.json` files
...
- [ ] CHK033 — Are there requirements ensuring that adding 6 new function cases to the parser
...
- [ ] CHK034 — Is the assumption "no TUI changes required beyond the help document" validated
...
- [ ] CHK035 — Is the assumption that `FormulaEvaluator.ParseFactor` is the only extension
...
- [ ] CHK036 — Is the assumption that `MidpointRounding.AwayFromZero` equals "half-up" for all
