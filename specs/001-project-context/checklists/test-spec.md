...
- [ ] CHK001 — Do the test tasks specify whether new `[Theory]` golden cases should be added
...
- [ ] CHK002 — Do the test tasks specify whether to use `[MemberData(nameof(...))]` (the
...
- [ ] CHK003 — Do the test tasks specify that the success-case data tuple requires **three**
...
- [ ] CHK004 — Do the test tasks specify test method naming following the existing convention
...
- [ ] CHK005 — Do the test tasks confirm that the existing floating-point assertion
...
- [ ] CHK006 — Do the test tasks specify the Spreadsheet cell initialization API for
...
- [ ] CHK007 — For T008's sparse AVERAGE test ("A2 empty"), is "empty" defined precisely as the
...
- [ ] CHK008 — For T006's cyclic-reference `[Fact]` test, is the exact cell setup API specified
...
- [ ] CHK009 — For T011's "all-text range" COUNT test, is the method for creating a text cell
...
- [ ] CHK010 — For T017's IF equality test `IF(A1=A2,10,20)`, are both A1 and A2 values
...
- [ ] CHK011 — For T017's IF not-equal test `IF(A1<>0,A1,0)`, is A1's specific value and
...
- [ ] CHK012 — For T017's range-sum condition test `IF(A1>B5,1,0)`, are the values for all
...
- [ ] CHK013 — For T006's MIN single-cell test, is "cell value" replaced with a concrete
...
- [ ] CHK014 — For T014's ROUND composition test, is the expected output clarified as the
...
- [ ] CHK015 — For T017's nested-branch test `IF(A1>0,MAX(A1>A3),MIN(A1>A3))`, are A1, A2,
...
- [ ] CHK016 — For T011's COUNT sparse range test, is it explicitly stated which cells hold
...
- [ ] CHK017 — For T008's rectangular AVERAGE test `AVERAGE(A1>B2)=2.5`, are all four cell
...
- [ ] CHK018 — Do the test tasks list the exact German error message fragment for each `[Fact]`
...
- [ ] CHK019 — For T006's cyclic-reference error, is the exact fragment specified as
...
- [ ] CHK020 — For T017's "no relational operator" error test `IF(A1,1,0)`, is A1's value
...
- [ ] CHK021 — Do the error test tasks specify whether `result.ErrorPosition >= 1` should be
...
- [ ] CHK022 — Does T017a specify that `result.Success` (the return value of `Recalculate()`)
...
- [ ] CHK023 — Does T017a specify the exact `EditCell` API call for entering the IF formula in
...
- [ ] CHK024 — Is an inverted-column-order range test (e.g., `MIN(B1>A5)` where column B >
...
- [ ] CHK025 — Does T017 include test rows for the `<=` (less-than-or-equal) and `>=`
