...
- Lern- und nutzerseitige Texte stehen auf Deutsch zuerst und Englisch danach und zielen auf CEFR B2.
- Status, Abhängigkeiten, Entscheidungen und nächste Schritte bleiben text-first sowie soweit anwendbar nach WCAG 2.2 Level AA für Braillezeilen, Screenreader und Textbrowser vollständig.
- Für Lastenhefte, die nach dem Profil `.specify/templates/intake-authoring-level2-lastenheft-profile.md` erstellt oder gepflegt werden, gilt: Wenn mindestens drei zusammenhängende Abhängigkeiten, Zustände, Übergaben, Verzweigungen oder geordnete Schritte wesentlich von einer Visualisierung profitieren, folgt direkt nach der vollständigen text-first-Erklärung ein knappes Mermaid-Diagramm. Das Diagramm bleibt ergänzend; sein lesbarer Markdown-Quelltext wird versioniert und darf keine Information tragen, die im Begleittext fehlt. Unmittelbar darunter fasst eine kurze Textalternative die wesentlichen Beziehungen und gegebenenfalls Entscheidungszweige auf Deutsch zuerst und Englisch danach zusammen. Ein einfaches Intake ohne wesentlichen Klarheitsgewinn dokumentiert `N/A`.
- Öffentliche C#-Typen und -Mitglieder erhalten `<summary>` sowie alle fachlich anwendbaren `<param>`, `<returns>` und `<exception>`; lokale Variablen sind keine XML-Dokumentationsziele.
- CS1591 bleibt eine Build-Schranke und wird nicht global unterdrückt. Änderungen an API-Signaturen oder XML-Kommentaren regenerieren DocFX und lösen denselben textorientierten A11Y-Smoke aus.
- Neue oder geänderte nicht triviale Logik erhält in moderater Dichte zweisprachige Warum-Kommentare zu Entscheidungen, Abwägungen, Grenzen oder historischen Abweichungen.
- Künftige Funktionen und Fehlerkorrekturen belegen Rot → Grün → Aufräumen mit konkreter Testdatei, beobachtbarem roten Fehler, grüner Implementierungsdatei sowie Regression/Aufräumen.
- Reine Governance-/Textarbeit darf TDD und Changed-Code-Coverage nur mit Begründung und Wiedervorlage `N/A` setzen. Bei geändertem Produktcode gelten mindestens 70 Prozent Coverage und das Ziel 80 Prozent.

*Learner-facing and user-facing text is German first and English second at CEFR B2. Status, dependencies, decisions, and next actions remain complete in text and, where applicable, meet WCAG 2.2 Level AA for Braille displays, screen readers, and text browsers. For requirement documents created or maintained under `.specify/templates/intake-authoring-level2-lastenheft-profile.md`, if three or more connected dependencies, states, handoffs, branches, or ordered steps materially benefit from visualization, a concise Mermaid diagram follows the complete text-first explanation; the version-controlled Markdown source remains readable and carries no information absent from the accompanying text. A short text alternative immediately below summarizes the essential relationships and, where applicable, decision branches in German first and English second. A simple intake without material clarity gain records `N/A`. Public C# types and members receive `<summary>` plus every technically applicable `<param>`, `<returns>`, and `<exception>`; local variables are not XML documentation targets. CS1591 remains an unsuppressed build gate, and API-signature or XML-comment changes regenerate DocFX with the same text-oriented accessibility smoke check. New or changed non-trivial logic uses moderate bilingual why-comments. Future features and fixes provide exact red, green, regression, and refactor evidence. Governance-only or text-only work may mark TDD and changed-code coverage `N/A` only with a rationale and re-evaluation trigger; changed product code has a 70% minimum and an 80% target.*
