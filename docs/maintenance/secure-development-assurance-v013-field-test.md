# Secure Development Assurance v0.1.3 – TinyCalc-Feldtest

## Ergebnis und Scope / Result and Scope

**Empfehlung: `ReleaseAccepted`.** TinyCalc bestätigt das Preset
...
Funktionsfähigkeit des unveränderten Presets im nichtkommerziellen Ausbildungs-
und Beispielprojekt. Sie ist weder Produktfreigabe noch Pilot-, Projekt-,
Risiko-, C5-, Konformitäts- oder Zertifizierungsentscheidung.

*Recommendation: `ReleaseAccepted`. TinyCalc confirms the unchanged
...
Produktcode, Produkt-API, Runtime, Abhängigkeiten und Bildartefakte wurden nicht
geändert.
...
| Roh-Hash-Snapshot vor/nach Status und Reviews | 6 von 6 Evidence-Dateien unverändert | 0 |
...
| Projektvalidator RL-SE, Bash und PowerShell | 157 von 157 IDs; bestanden | 0 |
| Projektvalidator GSDB, Bash und PowerShell | bestanden | 0 |
...
Image-Impact- und unzulässiger C5-/Zertifizierungsbehauptungen. Diese
synthetischen Fehler enden erwartungsgemäß mit Exitcode 2; der Gesamttest endet
nach erfolgreicher Prüfung mit Exitcode 0. Der C5-Negativfall ist ein
Fail-closed-Sicherheitstest und keine C5-Prüfung des Projekts.
...
## Feldtest-Befund / Field-Test Finding

v0.1.3 blockierte zunächst korrekt den historischen Kontext: Der
Verzeichnis-Suffix `rl-se-self-assessment` stimmte nicht mit der gespeicherten
vollständigen `contextId` `tinycalc-rl-se-self-assessment` überein. Der
projektgeführte Kontext wurde auf
`2026-09-05-tinycalc-rl-se-self-assessment` berichtigt und alle aktuellen
Pfad- und Hashbindungen wurden nachvollzogen. Der Preset-Validator wurde nicht
abgeschwächt. Nach der Korrektur finden beide Shells genau einen Kontext; alle
vier Reviews bleiben read-only. Daraus entsteht kein Preset-Patchbedarf.

*v0.1.3 initially blocked the historical context correctly because the
directory suffix did not match the stored full context ID. The project-owned
directory and all current path/hash bindings were corrected; the preset
validator was not weakened. Both shells now resolve exactly one context and all
four reviews remain read-only. This does not require a preset patch.*
...
- C5 ist für den aktuellen nichtkommerziellen Ausbildungs- und Beispielscope
  `N/A`; GitHub, CI und Artefakthosting sind Entwicklungsinfrastruktur, keine
  Produkt-Cloud-Runtime.
- CRA und formale Produktkonformität sind für diesen aktuellen Scope `N/A`.
...
bleibt eine dokumentierte CLI-Grenze; im hier verwendeten isolierten
Codex-Profil trat kein verwaister Claude-Pfad auf.

Offene Punkte dieses Feldtests: keine. Die Community-Einreichung
...
Feldbericht; die aktualisierten Security-Entscheidungen und der korrigierte
Evidence-Kontext sind dessen Nachweise. Zielgruppen sind Maintainer, technische
Reviewer und Lernende. Leserpfad: v0.1.3-Adoption → Feldbericht →
