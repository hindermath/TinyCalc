...
- Für Lastenhefte, die nach dem Profil `.specify/templates/intake-authoring-level2-lastenheft-profile.md` erstellt oder gepflegt werden, gilt: Wenn mindestens drei zusammenhängende Abhängigkeiten, Zustände, Übergaben, Verzweigungen oder geordnete Schritte wesentlich von einer Visualisierung profitieren, folgt direkt nach der vollständigen text-first-Erklärung ein knappes Mermaid-Diagramm. Das Diagramm bleibt ergänzend; sein lesbarer Markdown-Quelltext wird versioniert und darf keine Information tragen, die im Begleittext fehlt. Ein einfaches Intake ohne wesentlichen Klarheitsgewinn dokumentiert `N/A`.
