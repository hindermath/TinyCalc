...
- Lastenhefte ergänzen komplexe Abhängigkeiten, Zustände, Übergaben oder Ablaufreihenfolgen ab drei zusammenhängenden Elementen durch ein knappes Mermaid-Diagramm direkt nach der vollständigen text-first-Erklärung. Das Diagramm bleibt ergänzend; sein lesbarer Markdown-Quelltext wird versioniert und darf keine Information tragen, die im Begleittext fehlt.
...
*Learner-facing and user-facing text is German first and English second at CEFR B2. Status, dependencies, decisions, and next actions remain complete in text and, where applicable, meet WCAG 2.2 Level AA for Braille displays, screen readers, and text browsers. Requirement documents supplement complex relationships of three or more connected elements with a concise Mermaid diagram after the complete text-first explanation; the version-controlled Markdown source remains readable and carries no information absent from the accompanying text. Public C# types and members receive `<summary>` plus every technically applicable `<param>`, `<returns>`, and `<exception>`; local variables are not XML documentation targets. CS1591 remains an unsuppressed build gate, and API-signature or XML-comment changes regenerate DocFX with the same text-oriented accessibility smoke check. New or changed non-trivial logic uses moderate bilingual why-comments. Future features and fixes provide exact red, green, regression, and refactor evidence. Governance-only or text-only work may mark TDD and changed-code coverage `N/A` only with a rationale and re-evaluation trigger; changed product code has a 70% minimum and an 80% target.*
